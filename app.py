# Python 程式碼 V12.0 (React UI 左右分欄完整版)
...
# --- UI 生成函數 (HTML/CSS) ---
# 注意：這裡的 HTML 字串全部靠左對齊，不可縮排，以避免 Markdown 解析錯誤
def render_dashboard_html(day_stats, meal_stats, supp_list, med_list, record_date_str):
    icons = {
...
<div class="header-card">
    <div class="header-icon">{icons['calendar']}</div>
    <div>
        <div style="font-size:20px; font-weight:700; color:#1e293b;">大文的飲食日記</div>
        <div style="font-size:14px; color:#64748b;">{record_date_str}</div>
    </div>
</div>
"""
...
<div class="dashboard-card">
    ...
    <div style="margin-top:20px; padding-top:16px; border-top:1px solid #f1f5f9;">
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
            <div><div style="font-size:11px; font-weight:700; color:#94a3b8; margin-bottom:8px; text-transform:uppercase;">保養品</div><div class="tag-container">{get_tag_html(supp_list, "tag-green", "leaf")}</div></div>
            <div><div style="font-size:11px; font-weight:700; color:#94a3b8; margin-bottom:8px; text-transform:uppercase;">藥品</div><div class="tag-container">{get_tag_html(med_list, "tag-red", "pill")}</div></div>
        </div>
    </div>
</div>
"""
    # 本餐 (小計)
    meal_html = f"""
<div class="dashboard-card" style="padding:16px; margin-bottom:16px; background:#f8fafc; border:1px solid #e2e8f0;">
    <div class="section-title" style="font-size:14px; margin-bottom:12px;">
        <div class="section-icon bg-blue" style="padding:4px;">{icons['utensils']}</div>本餐營養小計
    </div>
    <div class="grid-stats" style="gap:8px;">
        ...
        {get_stat_html("beef", "蛋白", f"{meal_stats['prot']:.1f}", "g", "bg-red", "#ef4444")}
        {get_stat_html("dna", "脂肪", f"{meal_stats['fat']:.1f}", "g", "bg-yellow", "#eab308")}
    </div>
</div>
"""
    return style, header_html, daily_html, meal_html
...
# --- 2. 餐別設定 (邏輯放在 Layout 之前，確保 meal_name 正確) ---
recorded_meals = []
if not df_today.empty:
...
# [重要] 這裡不渲染，只獲取 meal_name 用於後續計算
meal_name_current = st.session_state.meal_selector

# --- 3. 計算本餐數據 (供 Dashboard 使用) ---
last_bowl = 30.0
df_meal = pd.DataFrame()
if not df_today.empty:
    mask_meal = (df_today['Meal_Name'] == meal_name_current)
...
meal_stats['name'] = meal_name_current
if not df_meal.empty:
    for col in ['Cal_Sub', 'Net_Quantity', 'Prot_Sub', 'Fat_Sub']:
...
# --- 4. 渲染 HTML Dashboard + 標題 ---
style_css, header_html, daily_html, meal_html = render_dashboard_html(day_stats, meal_stats, supp_list, med_list, record_date.strftime('%Y年 %m月 %d日'))
...
with col_right:
    ...
    with c2:
        st.markdown("##### 🥣 碗重 (g)")
        bowl_weight = st.number_input("碗重", value=last_bowl, step=0.1, format="%.1f", label_visibility="collapsed")
    ...
    # 3. 操作 Tabs
    st.write("") # Spacer
    nav_mode = st.radio("Mode", ["➕ 新增食物/藥品", "🏁 完食/紀錄剩餘"], horizontal=True, label_visibility="collapsed", key="nav_mode")
